...
[English](backend-railway.readme.md) | 简体中文
...
本指南包含 Railway 配置和云端验证步骤；本地运行见 [README-zh.md](../backend/README-zh.md)，完整对话协议见 [对话协议说明](../backend/docs/stage2-usage-zh.md)。

页面启动与演示见 [本地演示说明](../backend/docs/stage3-usage-zh.md)，前端部署见 [前端 Railway 指南](frontend-railway.readme-zh.md)。
...
将它和后端代码、`data/catalog.json`、`images/item-*.jpg` 一起提交到准备部署的分支。
...
也可使用 `POST /api/bookings` 验证结构化预约；其完整字段见 [README-zh.md](../backend/README-zh.md)。
